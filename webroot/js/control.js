						...
						d3.json("./data/fl1p.geojson", function (json) {
							...
							d3.json("./data/fl2p.geojson", function (json) {
								...
								d3.json("./data/fl3p.geojson", function (json) {
									...
									document.getElementById("floor" + curFloor).style.opacity = .9;
...
function doLocate(d) {
	...
	curLoc = XY;
	doZoom(0); //trigger redrawing & centering
	webSockConn.send('storeLocation:' + curLoc[0] + ';' + curLoc[1] + ';' + curFloor); // Send the message to the server
}

function changeFloor(floor) {
	document.getElementById("floor0").style.opacity = .3;
	document.getElementById("floor1").style.opacity = .3;
	document.getElementById("floor2").style.opacity = .3;
	document.getElementById("floor3").style.opacity = .3;
	curFloor = floor;
	document.getElementById("floor" + floor).style.opacity = .9;
